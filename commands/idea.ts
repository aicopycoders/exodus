...
const SOURCES = ["gambit", "organic", "swipe"] as const;
type Source = (typeof SOURCES)[number];

const KEY_RE = /^[GOSgos]\d+$/;

/** Split "G1,S4, O2" into validated keys, or null if any is malformed/empty. */
export function parseKeys(raw: string): string[] | null {
  const parts = raw.split(",").map((s) => s.trim()).filter((s) => s.length > 0);
  if (parts.length === 0) return null;
  for (const p of parts) if (!KEY_RE.test(p)) return null;
  return parts.map((p) => p.toUpperCase());
}

export type IdeaAction =
  | { kind: "add"; hook: string; description: string; source: Source; sourceRef?: string; notes?: string }
...
/**
 * Resolve a variant count from flags. The friendly knob is --passes (writing
 * passes per bot, 1 pass = 1 Mario + 1 Infeed = 2 variants). --variants is an
 * advanced raw total override that wins when both are given. parseInt("0") is a
 * valid 0, so NaN is guarded explicitly or `--passes 0`/`--variants 0` would
 * fall through to the default instead of clamping to the minimum.
 */
function resolveVariantCount(flags: Record<string, string | boolean>): number {
...
export function resolveIdeaAction(
  positionals: string[],
  ...
  if (sub === "add") {
    ...
    if (!hook) return { kind: "error", message: 'add needs a hook: exodus idea add "<hook>"' };
    const sourceRaw = str(flags["source"]) ?? "gambit";
    if (!SOURCES.includes(sourceRaw as Source)) {
  ...
  if (sub === "list") {
    ...
    if (sourceRaw && !SOURCES.includes(sourceRaw as Source)) {
      return { kind: "error", message: `--source must be one of: ${SOURCES.join(", ")}` };
    }
    const limitRaw = str(flags["limit"]);
    return {
      kind: "list",
      source: sourceRaw as Source | undefined,
  ...
  if (sub === "note") {
    ...
    if (!key || !notes) return { kind: "error", message: 'note needs a key + text: exodus idea note G1 "..."' };
    if (!KEY_RE.test(key)) return { kind: "error", message: `Bad key "${key}"` };
  ...
  if (sub === "edit") {
    ...
    if (!key || !description) return { kind: "error", message: 'edit needs a key + concept: exodus idea edit G1 "..."' };
    if (!KEY_RE.test(key)) return { kind: "error", message: `Bad key "${key}"` };
  ...
  if (sub === "write") {
    ...
    if (!keys) return { kind: "error", message: 'write needs keys: exodus idea write G1,S4' };
    // Variant count: friendly knob is --passes (writing passes per bot, 1 pass =
    // 1 Mario + 1 Infeed = 2 variants). --variants is an advanced raw total override
    // that wins when both are given. parseInt("0") is a valid 0, so guard NaN
    // explicitly or `--passes 0`/`--variants 0` falls through to the default.
    return {
      kind: "write",
      keys,
  ...
  if (sub === "rm") {
    const key = positionals[1];
    if (!key || !KEY_RE.test(key)) return { kind: "error", message: 'rm needs a key: exodus idea rm G1' };
    return { kind: "rm", key: key.toUpperCase(), hard: flags["hard"] === true };
  }

  if (sub === "gambit") {
    const dump = positionals.slice(1).join(" ").trim();
    if (!dump) return { kind: "error", message: 'gambit needs a brain-dump: exodus idea gambit "<text>"' };
    return { kind: "gambit", dump };
  }

  if (sub === "organic") {
    const urls = positionals.slice(1).join(" ").split(/\s+/).map((u) => u.trim()).filter(Boolean);
    if (urls.length === 0) return { kind: "error", message: 'organic needs at least one URL: exodus idea organic "<url> <url>"' };
    return {
      kind: "organic",
      urls,
...
/** parseArgs drops positionals; recover them (mirror genesis.ts). */
function parsePositional(): string[] {
  const args = process.argv.slice(3); // drop node, script, "idea"
  const out: string[] = [];
  let i = 0;
  while (i < args.length) {
    const a = args[i];
...
interface IdeaRow {
  ...
  status?: string;
  notes?: string | null;
  outputDocUrl?: string | null;
  createdAt?: string;
}

function renderList(ideas: IdeaRow[]): void {
  if (ideas.length === 0) {
  ...
  for (const i of ideas) {
    ...
    console.log(`      (${meta})`);
    if (i.notes) console.log(`      notes: ${i.notes}`);
    if (i.outputDocUrl) console.log(`      doc: ${i.outputDocUrl}`);
  }
  console.log("");
...
export async function run(flags: Record<string, string | boolean>): Promise<void> {
  ...
  switch (action.kind) {
    ...
    case "gambit": {
      const res = await apiPost<{ batchId?: string; jobId?: string; error?: string }>(
        "/api/v2/idea-bank/ideate",
        { source: "gambit", input: action.dump },
        { ccCommand: cc },
      );
    ...
    case "organic": {
      // --write: transcribe → bank → write each reel through Genesis in one shot.
      if (action.write) {
      ...
      // Default: capture only (curate in the bank, write later).
      const res = await apiPost<{ batchId?: string; jobId?: string; error?: string }>(
        "/api/v2/idea-bank/ideate",
        { source: "organic", urls: action.urls },
        { ccCommand: cc },
      );
      if (!res.ok) { console.log(formatError(res)); process.exit(1); }
    ...
    case "swipe": {
      const res = await apiPost<{ batchId?: string; jobId?: string; error?: string }>(
        "/api/v2/idea-bank/ideate",
        { source: "swipe", limit: action.limit },
        { ccCommand: cc },
      );
      if (!res.ok) { console.log(formatError(res)); process.exit(1); }
    ...
    case "add": {
      const res = await apiPost<{ key?: string; error?: string }>(
        "/api/v2/idea-bank",
        {
    ...
    case "list": {
      ...
      const res = await apiGet<{ ideas?: IdeaRow[]; error?: string }>(
        `/api/v2/idea-bank${qs ? `?${qs}` : ""}`,
      );
      if (!res.ok) { console.log(formatError(res)); process.exit(1); }
    ...
    case "note": {
      const res = await apiPost<{ ok?: boolean; error?: string }>(
        ...
        { key: action.key, notes: action.notes },
        { ccCommand: cc },
      );
      if (!res.ok) { console.log(formatError(res)); process.exit(1); }
    ...
    case "edit": {
      const res = await apiPost<{ ok?: boolean; error?: string }>(
        "/api/v2/idea-bank/update",
        { key: action.key, description: action.description },
        { ccCommand: cc },
      );
      if (!res.ok) { console.log(formatError(res)); process.exit(1); }
    ...
    case "rm": {
      const res = await apiPost<{ ok?: boolean; error?: string }>(
        "/api/v2/idea-bank/delete",
        { key: action.key, hard: action.hard },
        { ccCommand: cc },
      );
      if (!res.ok) { console.log(formatError(res)); process.exit(1); }
