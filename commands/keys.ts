...
async function status(): Promise<void> {
  ...
  try {
    ...
    console.error(`Could not reach the dashboard: ${(e as Error).message}`);
    process.exit(1);
    return;
  }
...
async function pull(): Promise<void> {
  ...
  if (count === 0) {
    console.log(
      "No keys are set in the dashboard yet. Add them at Settings → Pipeline Keys, then run this again.",
    );
    if (remote.failed.length) {
...
export async function run(): Promise<void> {
  // process.argv[2] = "keys", process.argv[3] = sub-action
