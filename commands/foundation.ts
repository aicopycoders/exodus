...
// 2-track Genesis methodology: 4 awareness levels collapse into 2 primers.
const FIELD_ORDER = [
  "audienceConcerns",
  "primerUnawareProblemAware",
  "primerSolutionProductAware",
...
async function fetchStatus(): Promise<StatusResp> {
  const res = await apiGetDashboard<StatusResp | ApiError>(
...
async function pickSource(flags: Record<string, string | boolean>): Promise<{
  kind: "googleDoc" | "url" | "text";
  value: string;
} | null> {
  ...
  const choice = await promptChoice(
    "Where should we pull the brand info from?",
    [
  ...
  if (choice === "3") {
    ...
    if (!filePath) fail("no file path provided");
  ...
  if (!text.trim()) fail("no text entered");
  return { kind: "text", value: text };
}
...
async function fetchSource(
  ...
  const res = await apiPostDashboard<SourceResp | ApiError>(
...
async function extract(
  ...
  const res = await apiPostDashboard<ExtractResp | ApiError>(
    "/api/foundation/extract",
    { fieldKey, sourceText },
  );
  if (!res.ok) {
  ...
  return res.data as ExtractResp;
}

async function saveField(fieldKey: string, value: string): Promise<SetResp> {
  const res = await apiPostDashboard<SetResp | ApiError>(
    "/api/foundation/set",
    { fieldKey, value },
  );
  if (!res.ok) {
...
async function walkField(
  ...
  if (field.populated) {
    const regen = await promptYesNo(
      `${field.label} is already populated (${field.chars} chars). Regenerate?`,
      false,
    );
    if (!regen) {
  ...
  for (;;) {
    ...
    const choice = await promptChoice(
      `What do you want to do with this ${field.label} suggestion?`,
      [
        { key: "a", label: "accept" },
...
// 2-track Genesis methodology field keys. Legacy single-level keys
// (primerUnaware / primerProblemAware / primerSolutionAware /
// primerProductAware) are still accepted by setBrandFoundationFieldByApiKey
// on the server, where they collapse into the 2-track shape — but the CLI
// only emits the 2-track keys directly.
...
/**
 * Map a markdown H2 header to a foundation field key. Tolerates dashes,
 * parens, casing, and "Aware" / "-Aware" / " Aware" variants so a Doc
 * authored by hand or by an LLM lands cleanly.
 *
 * After the 4→2 primer collapse: any header that pairs unaware/problem-
 * aware (or "cold" / "TOFU") routes to primerUnawareProblemAware; any
 * header that pairs solution-aware/product-aware (or "warm" / "MOFU" /
 * "BOFU") routes to primerSolutionProductAware. Single-level legacy
 * headers ("Unaware", "Problem-Aware", etc.) still parse and route to
 * the 2-track destination so old docs don't break.
 *
 * Returns null when the header is something else (e.g. a sub-heading).
 */
function headerToFieldKey(rawHeader: string): string | null {
  ...
  // 2-track headers. Match anything that names BOTH halves of a track or
  // the cold/warm shorthand. Check these first so paired headers aren't
  // shadowed by the single-level fallbacks.
  ...
  if ((hasUnaware && hasProblem) || hasCold) return "primerUnawareProblemAware";
  if ((hasSolution && hasProduct) || hasWarm) return "primerSolutionProductAware";

  // Single-level headers — route to the 2-track destination.
  if (norm.startsWith("primer")) {
    if (hasUnaware) return "primerUnawareProblemAware";
    if (hasProblem) return "primerUnawareProblemAware";
    if (hasSolution) return "primerSolutionProductAware";
    if (hasProduct) return "primerSolutionProductAware";
  }
  if (norm === "unaware") return "primerUnawareProblemAware";
  if (norm.startsWith("problem aware")) return "primerUnawareProblemAware";
  if (norm.startsWith("solution aware")) return "primerSolutionProductAware";
  if (norm.startsWith("product aware")) return "primerSolutionProductAware";
...
function parseFoundationMarkdown(md: string): ParsedSection[] {
  ...
  // Dedup: if two sections map to the same field, last one wins.
  const byField = new Map<string, ParsedSection>();
  for (const s of sections) byField.set(s.fieldKey, s);
  return [...byField.values()];
}

function finalizeSection(s: ParsedSection): ParsedSection {
...
async function runSave(filePath: string): Promise<void> {
  if (!filePath) {
    fail(
      "missing path. usage: exodus foundation save <path>  (use \"-\" to read from stdin)",
    );
  }
  let md: string;
  if (filePath === "-") {
  ...
  for (const s of sections) console.log(`  - ${s.fieldKey} (from "${s.headerLine}")`);
  console.log("");
...
async function runSet(
  ...
  if (!fieldKey) fail("missing field. usage: exodus foundation set <field> --value <text> | --file <path>");
  if (!FIELD_KEYS.has(fieldKey)) {
    fail(
      `invalid field "${fieldKey}". must be one of: ${[...FIELD_KEYS].join(", ")}`,
    );
  }

  let value: string | null = null;
  if (typeof flags.value === "string") value = flags.value;
  ...
  if (value === null || !value.trim()) {
    fail(
      "missing or empty content. provide --value <text> | --file <path> | --stdin (or --file -).",
    );
  }
...
async function runInteractive(
  ...
  console.log(
    `Source: ${sourceResp.label} — ${sourceResp.chars} chars${truncNote}`,
  );
