export function formatIntelResult(data) {
    ...
    if (data["adsAnalyzed"] !== undefined)
        lines.push(`**Ads Analyzed:** ${data["adsAnalyzed"]}`);
    if (data["winnersFound"] !== undefined)
        lines.push(`**Winners Found:** ${data["winnersFound"]}`);
    if (data["losersFound"] !== undefined)
        lines.push(`**Losers Found:** ${data["losersFound"]}`);
    const attempted = data["combinationsAttempted"];
    const succeeded = data["combinationsSucceeded"];
    const total = data["combinationsCount"];
    if (total !== undefined)
        lines.push(`**Combinations (total):** ${total}`);
