...
async function listOrganic() {
    const res = await apiGet("/api/v2/idea-bank?source=organic&limit=200");
    if (!res.ok)
        return [];
    return Array.isArray(res.data.ideas) ? res.data.ideas : [];
}
export async function captureReelAndWrite(urls, opts, rt = {}) {
    ...
    const before = new Set((await listOrganic()).map((r) => r.key).filter(Boolean));
    const ideate = await apiPost("/api/v2/idea-bank/ideate", { source: "organic", urls: wanted }, { ccCommand: rt.cc });
    if (!ideate.ok) {
