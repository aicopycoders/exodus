...
export function findMergeConflicts(src, dest) {
...
export async function run(flags) {
    ...
    const conflicts = [
        ...findMergeConflicts(path.join(root, "state"), path.join(brandDir, "state")).map((f) => path.join("state", f)),
        ...findMergeConflicts(path.join(root, "output"), path.join(brandDir, "output")).map((f) => path.join("output", f)),
    ];
