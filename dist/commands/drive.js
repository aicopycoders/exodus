...
function readPositionals() {
    ...
    return argv.filter((a, i) => !a.startsWith("--") &&
...
async function getDoc(documentId) {
    if (!documentId)
        fail("usage: exodus drive get-doc <documentId>");
    const res = await apiPostDashboard("/api/drive/docs/get", { documentId });
    printOrFail(res);
}
async function createDoc(title, flags) {
    if (!title)
        fail("usage: exodus drive create-doc <title> [--folder <id>]");
...
async function batchUpdate(documentId, flags) {
    if (!documentId) {
...
async function listFiles(flags) {
    const body = {};
    ...
    if (typeof flags["order-by"] === "string")
        body.orderBy = flags["order-by"];
    const res = await apiPostDashboard("/api/drive/files/list", body);
    printOrFail(res);
}
async function getSheet(spreadsheetId, flags) {
    if (!spreadsheetId) {
